...
The train dataset for the TYPED-SIMILARITY PILOT contains the following:

  00-README.txt 		  this file
  correlation-typed.pl		  evaluation script
  ...
  visualizer.pl		  	  perl script to generate HTML files for 
                                  better visualization of the dataset pairs
                                  (optional)

The pull-records script will produce the following directories and files:
...
The visualizer script will produce a directory with one html file for
each pair. It needs to be run after pull-records.pl.
...
between cultural heritage items, which contain several fields with
textual values. Systems should return a general similarity score, plus
an additional score for each type of similarity considered:
...
Please follow these instructions to retrieve the data for the train
dataset.
...
You should then see the following directories and files:

  text/*.txt                      textual record of each item
  json/*.json                     full json information of each item
  images/*                        image of each item

Now you can run visualizer.pl and get HTML files to visualize the
pairs (optional):
...
Please check STS.output.europeana.txt which returns a random number for all
