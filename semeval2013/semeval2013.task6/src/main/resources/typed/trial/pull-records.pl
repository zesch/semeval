...
my $failed = 0;
my $crashed = 0;
open(PAIRSFILE, "<STS.input.europeana.txt") or die "Cannot open < typed-similarity_pairs.txt: $!";
open(ERRORFILE, ">pull-record-errors.txt") or die "Cannot open > errors.txt: $!";
...
sub fetch_json_page {
	...
	if ($json_text->{success}) {
	    ...
	    $failed++;
	    print "Failed!\n";
	    print ERRORFILE "Error extracting $num item.\n";
	}	    
    };
    # catch crashes:
    if($@){
	...
	print ERRORFILE "[[JSON ERROR]] JSON parser crashedin $num item! $@\n";
    }
}
